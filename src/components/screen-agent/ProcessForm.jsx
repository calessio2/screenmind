...
export default function ProcessForm({ open, onClose, onSaved, editingProcess }) {
  ...
  const handleAIGenerated = (data) => {
    ...
    setSteps(data.steps?.length ? data.steps : [{ title: "", description: "", image_url: "" }]);
    setMode("manual");
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto bg-zinc-900 border-zinc-800">
        ...
        {!editingProcess && (
          <div className="flex gap-1 p-1 bg-zinc-800/50 rounded-lg">
            ...
            <button
              onClick={() => setMode("ai")}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 rounded-md text-xs font-medium transition-colors ${
                mode === "ai" ? "bg-purple-600/20 text-purple-300" : "text-zinc-500 hover:text-zinc-300"
              }`}
            >
        ...
        {mode === "ai" && !editingProcess ? (
          <AIImportPanel onGenerated={handleAIGenerated} onClose={onClose} />
        ) : (
